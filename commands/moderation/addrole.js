...
run: async (bot, message, args) => {
        ...
        if (!message.member.hasPermission("MANAGE_ROLES")) return message.lineReply("\```yml\nMODERATION: You need MANAGE_ROLES permission!\n\```");
        if (!message.guild.me.hasPermission("MANAGE_ROLES")) return message.lineReply("\```yml\nMODERATION: I do not have enough permissions!\n\```");
        
        if (!args[0]) return message.lineReply("\```yml\nMODERATION: Please mention a user!\n\```")
        ...
        if (!rMember) return message.lineReply("\```yml\nMODERATION: Please enter a username!\n\```");
        if (rMember.roles.highest.comparePositionTo(message.guild.me.roles.highest) >= 0) return message.lineReply('\```yml\nMODERATION: Cannot add the role to that user!\n\```')
        ...
        if (!args[1]) return message.lineReply("\```yml\nMODERATION: Please enter a role!\n\```")

        if (!role) return message.lineReply("\```yml\nMODERATION: Could not find that role!\n\```")

        if (role.managed) return message.lineReply("\```yml\nMODERATION: Cannot add the role to that user!\n\```")
        if (message.guild.me.roles.highest.comparePositionTo(role) <= 0) return message.lineReply('\```yml\nMODERATION: Roles are currently higher than me!\n\```')

        if (rMember.roles.cache.has(role.id)) return mmessage.lineReply("\```yml\nMODERATION: User already has the role!\n\```")
