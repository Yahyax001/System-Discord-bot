...
module.exports = {
    ...
    run: async (client, message, args) => {
      if (!message.member.hasPermission("MANAGE_EMOJIS")) {
            return message.lineReply(`:x: You Don't Have Permission To Use This Command!`)
        }
let emojis = await fetch("https://emoji.gg/api/").then(res => res.json());
     ...
     if (!matches.length) return message.lineReply(noResult)
     let page = 0;
     ...
     catch { message.lineReply(message.author.toString() + ", You took too long."); msg.reactions.removeAll() ; doing = false; return; };
...
rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
      ...
      try { 
        ...
        message.channel.stopTyping();
        message.lineReply(`Unable to add ${res.title}.`);
        rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
        doing = false;
        break;
       }
       message.lineReply(`Successfully added ${created}!`);
      rmsg.reactions.resolve(reaction.emoji.name).users.remove(message.author.id).catch(err => {})
       doing = false;
       break;
 
     } else if (reaction.emoji.name == emojis[3]) {
      message.lineReply("Cancelled command.");
